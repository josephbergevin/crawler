...
/*Joe Bergevin:
	file_get_html does not pass the response headers array (even though it is created) when it creates the dom document.
	This is a modified version of file_get_html. Function returns an array with 2 values:
		1. If html document exists, returns the dom document of it and the response header array
		2. If html document doesn't exist, returns false and the response header array
	To make it work the same way as file_get_html:
		$html = file_get_html( $url );
		$response_headers = $html[1];
		$html = html[0];
	You can then use the variable $html in the way it was orignally intended (e.g. $html->plaintext, etc).
*/
function file_get_html_with_headers($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT)
{
    // We DO force the tags to be terminated.
    $dom = new simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $defaultBRText);
    // For sourceforge users: uncomment the next line and comment the retreive_url_contents line 2 lines down if it is not already done.
...
//    $contents = retrieve_url_contents($url);
    if (empty($contents))
    {
        return array( false, $http_response_header );
    }
    // The second parameter can force the selectors to all be lowercase.
    $dom->load($contents, $lowercase, $stripRN);
    //return $dom;
	return array( $dom, $http_response_header );
}
